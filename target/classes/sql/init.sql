...
CREATE TABLE `knowledge_base` (
  ...
  `scene` VARCHAR(100) DEFAULT NULL COMMENT '场景标识（如：通用/删除产品/替换产品等）',
...
-- 插入修改类型规则数据
INSERT INTO `modification_type_rule` (`type`, `keywords`, `scene`, `rule_detail`, `priority`) VALUES
('删除', '删除,删掉,移除,去掉,去除', '通用', '1. 删除产品时，需从"产品包含"字段的full_name_list中删除该产品名称；2. 同步删除相关规则；3. 保留其他未提及内容不变', 100),
('新增', '新增,增加,添加,加入,加上', '通用', '1. 新增产品时，需在"产品包含"字段的full_name_list中添加该产品名称；2. 根据上下文添加相关规则（如产品替换规则）；3. 保持格式一致性', 90),
('修改', '修改,更改,调整,变更', '通用', '1. 修改指定内容为新值；2. 保持上下文语境一致；3. 同步更新所有相关引用', 80),
('替换', '替换,换成,改为,改成', '通用', '1. 将指定内容A替换为B；2. 全局替换所有出现处；3. 确保语义一致性', 85),
('加回', '加回,恢复,还原,放回', '通用', '1. 恢复之前删除的内容；2. 恢复相关规则；3. 保持与原始版本一致', 75),

-- 咖喱脆皮鸡上新需删除黄金脆皮鸡
('删除', '咖喱脆皮鸡上新,删除黄金脆皮鸡,黄金脆皮鸡', '咖喱脆皮鸡上新删除黄金脆皮鸡', '当操作指令为“咖喱脆皮鸡上新需删除黄金脆皮鸡”时：1）在产品包含区域，删除所有包含“黄金脆皮鸡（1块装）”的产品项；2）删除文案中关于黄金脆皮鸡的替换规则，例如“在不售卖黄金脆皮鸡的餐厅，产品将替换为吮指原味鸡。在不售卖吮指原味鸡的餐厅，产品将替换为黄金脆皮鸡；”; 3）保留原味鸡/脆皮鸡出餐部位随机等通用规则不变。', 110),

-- 咖喱脆皮鸡断货需加回黄金脆皮鸡
('加回', '咖喱脆皮鸡断货,加回黄金脆皮鸡,黄金脆皮鸡', '咖喱脆皮鸡断货加回黄金脆皮鸡', '当操作指令为“咖喱脆皮鸡断货需加回黄金脆皮鸡”时：1）在产品包含区域，新增“黄金脆皮鸡（1块装）”作为可选产品；2）在替换规则中，恢复关于黄金脆皮鸡的规则，例如“在不售卖黄金脆皮鸡的餐厅，产品将替换为吮指原味鸡。在不售卖吮指原味鸡的餐厅，产品将替换为黄金脆皮鸡；”; 3）保留原味鸡/脆皮鸡出餐部位随机等通用规则不变。', 105),

-- CNY期间：黄金脆皮鸡改名为避风塘黄金脆皮鸡
('替换', 'CNY,春节,黄金脆皮鸡改名,避风塘黄金脆皮鸡,改名', 'CNY期间黄金脆皮鸡改名为避风塘黄金脆皮鸡', '当操作指令为“CNY期间黄金脆皮鸡改名为避风塘黄金脆皮鸡”时：1）在产品包含区域，将所有“黄金脆皮鸡（1块装）”替换为“避风塘黄金脆皮鸡（1块装）”；2）在替换/后备规则中，将涉及黄金脆皮鸡的规则改为对应的避风塘黄金脆皮鸡规则，例如在不售卖避风塘黄金脆皮鸡时的替代产品说明；3）如文案中存在春节期间的特殊后备规则（例如售罄时可替换为新奥尔良烤翅2块装），按照图片中的规则说明补充或更新；4）其他与春节无关的通用规则保持不变。', 95);
