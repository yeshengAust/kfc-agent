...
public class KnowledgeBaseController {
    ...
    public ResponseEntity<Map<String, Object>> saveKnowledge(@RequestBody Map<String, String> request) {
        try {
            ...
            String scene = request.get("scene"); // 从前端获取场景
...
//            }
            ...
            if (existingKnowledge != null) {
                ...
                existingKnowledge.setScene(scene); // 更新场景
                knowledgeBaseRepository.updateKnowledgeBase(existingKnowledge);
                
                log.info("知识库已存在，覆盖更新，operation: {}, type: {}, scene: {}", operation, type, scene);
                ...
                knowledgeBase.setScene(scene); // 设置场景
                knowledgeBase.setResult(result);
                knowledgeBaseRepository.saveKnowledgeBase(knowledgeBase);

                log.info("成功添加知识库，operation: {}, type: {}, scene: {}", operation, type, scene);
                return ResponseEntity.ok(Map.of(
                    "success", true, 
                    "message", "添加成功"
                ));
